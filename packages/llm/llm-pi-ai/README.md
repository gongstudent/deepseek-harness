...
The three protocols a local (hand-declared) route can convert between are `openai-completions`, `openai-responses`, and `anthropic-messages`; every directed pair among them is representable as an `api` (outbound) / `inboundApi` (caller-facing) pair. `headers` sets provider request headers and `bodyOverrides` carries request-body keys a route forwards to the upstream, each value replacing the field pi-ai assembles. A route that records these fields is declaring what a local proxy should apply and how it should translate calls; the harness itself does not run an HTTP proxy, so the conversion is applied by the consumer of this profile rather than in the pi-ai adapter.
