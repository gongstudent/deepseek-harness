...
The three protocols a local route can convert between are `openai-completions`, `openai-responses`, and `anthropic-messages`; every directed pair among them is representable as an `api` (outbound, what the upstream speaks) / `inboundApi` (caller-facing, what the route serves) pair. `headers` sets provider request headers and `bodyOverrides` carries request-body keys a route forwards to the upstream, each value replacing the field the conversion assembles.

`localRoute: { enabled, port }` runs those conversions over loopback HTTP. Enabling it binds `127.0.0.1` on `port` — `8317` by default — and serves `POST /v1/chat/completions`, `POST /v1/responses`, and `POST /v1/messages`, where the path names the protocol the caller speaks, beside `GET /health`. A request selects the route whose `inboundApi` matches that path and whose catalog holds the body's `model`; when more than one route qualifies, the `x-dsh-provider` header names which, and its absence is refused with `400` rather than guessed. A model no route serves is `404`, and only a failing upstream is `502`. Changing the port rebinds, a failed bind keeps the previous listener serving, and disabling stops it.

Two limits are structural. A request whose inbound and outbound protocols are the same is forwarded verbatim, so its SSE stream reaches the caller untouched; a converted one cannot be, because each conversion needs a whole response, so the upstream call is made non-streaming and a `stream: true` caller receives the finished answer re-emitted as a synthetic event sequence — correct in content and framing, but arriving at once rather than progressively. The listener also holds no authentication of its own: it spends the selected route's configured credential on whatever reaches it, which is why it binds loopback and why every process on the host able to open that socket can spend that credential.
